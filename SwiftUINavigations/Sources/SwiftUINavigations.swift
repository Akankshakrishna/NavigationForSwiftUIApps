...
public class NavigationManager<T: Equatable>: ObservableObject {
    ...
    public init(initial: T? = nil, debug: Bool = false) {
        ...
        logger.log("SwiftUINavigation - Initialized.")
    ...
    public func push(_ route: T) {
        logger.log("SwiftUINavigation - Pushing \(route) route.")
    ...
    public func pop(animated: Bool = true) {
        if !self._routes.isEmpty {
            ...
            logger.log("SwiftUINavigation - \(popped) route popped.")
            onPopLast?(1, animated)
        }
    }

    public func popTo(_ route: T, inclusive: Bool = false, animated: Bool = true) {
        logger.log("SwiftUINavigation: Popping route \(route).")

        if _routes.isEmpty {
            logger.log("SwiftUINavigation - Path is empty.")
            return
        }

        guard var found = _routes.lastIndex(where: { $0 == route }) else {
            logger.log("SwiftUINavigation - Route not found.")
            return
        }
        ...
        logger.log("SwiftUINavigation - Popping \(numToPop) routes")
    ...
    public func onSystemPop() {
        if !self._routes.isEmpty {
            let popped = self._routes.removeLast()
            logger.log("SwiftUINavigation - \(popped) route popped by system")
        }
    }
}

public struct SwiftUINavigationsHost<T: Equatable, Screen: View>: View {
            
    @StateObject
    var navigationStyle = NavigationStyle()
        
    let manager: NavigationManager<T>
    @ViewBuilder
    let routeMap: (T) -> Screen

    public init(_ navManager: NavigationManager<T>, @ViewBuilder _ routeMap: @escaping (T) -> Screen) {
...
struct NavigationControllerHost<T: Equatable, Screen: View>: UIViewControllerRepresentable {
    ...
    let navManager: NavigationManager<T>
